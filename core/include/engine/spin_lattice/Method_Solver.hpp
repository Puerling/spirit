...
constexpr auto common_solver( SpinLattice::Solver solver ) -> Common::Solver
{
    switch( solver )
    {
...
constexpr auto name( SpinLattice::Solver solver ) -> std::string_view
{
    switch( solver )
    {
...
constexpr auto full_name( SpinLattice::Solver solver ) -> std::string_view
{
    switch( solver )
    {
        case Solver::None: return "None";
...
protected:
    ...
    virtual void Calculate_Force(
        const std::vector<std::shared_ptr<StateType>> & configurations, std::vector<quantity<vectorfield>> & forces )
        = 0;
...
#include <engine/spin_lattice/Solver_Heun.hpp>
#include <engine/spin_lattice/Solver_RK4.hpp>
#include <engine/spin_lattice/Solver_VP.hpp>
